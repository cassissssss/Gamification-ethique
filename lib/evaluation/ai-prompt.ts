...
export function buildAiPrompt(result: EvaluationResult, projectContext?: string): string {
  ...
  const riskThemes = getActiveRiskThemes(result)
   ...
   À faire : ${theme.recommendation ?? 'À clarifier.'}${signals ? `\n   Signaux :\n${signals}` : ''}`
    })
    .join('\n\n')
  ...
  const recommendations = result.positiveRecommendations
   ...
   À faire : ${recommendation.recommendation}${recommendation.example ? `\n   Exemple : ${recommendation.example}` : ''}`
    })
    .join('\n\n')
  ...
  const mechanics = result.mechanicsAlternatives
   ...
   Alternative : ${mechanic.ethicalAlternative}${mechanic.interfaceExample ? `\n   Exemple : ${mechanic.interfaceExample}` : ''}`
    })
    .join('\n\n')
...
${trimmedContext ? `- Un contexte de projet est fourni ci-dessous. Utilise-le activement : dans "ANGLES MORTS POSSIBLES" et "AJUSTEMENTS UX", relie au moins un point concret à ce contexte plutôt que de rester générique. Si ce contexte est vague ou incomplet (ex. pas de public cible précis), inclus une question dessus dans "QUESTIONS À POSER".` : ''}

---
${trimmedContext ? `\nCONTEXTE DU PROJET (fourni par l'équipe)\n\n${trimmedContext}\n\n---\n` : ''}
